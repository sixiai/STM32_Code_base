...
void LCD_Address_Set(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	if(USE_HORIZONTAL==0)
	{
		...
		LCD_Write_HalfWord(x1+26);
		LCD_Write_HalfWord(x2+26);
		LCD_Write_Cmd(0x2b);//行地址设置
		LCD_Write_HalfWord(y1+1);
		LCD_Write_HalfWord(y2+1);
	...
	{
		LCD_Write_Cmd(0x2a);//列地址设置
		LCD_Write_HalfWord(x1+1);
		LCD_Write_HalfWord(x2+1);
		LCD_Write_Cmd(0x2b);//行地址设置
		LCD_Write_HalfWord(y1+26);
		LCD_Write_HalfWord(y2+26);
		LCD_Write_Cmd(0x2c);//储存器写
	}
	else
	{
...
uint8_t SPI3_WriteData(uint8_t *data,uint16_t size)
{
	return HAL_SPI_Transmit(&hspi3,data,size, 1000);
}
